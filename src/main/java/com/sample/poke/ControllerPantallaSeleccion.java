...
public class ControllerPantallaSeleccion {
    ...
    public static File masculinoImg=new File("masculino.png");
    public static File hembraImg=new File("hembra.png");

    public static Image masculino = new Image(masculinoImg.toURI().toString());
    public static Image femenino =  new Image(hembraImg.toURI().toString());
    ...
    private void onButtonAbrirClicked(ActionEvent event) {
        try {
            ...
            if(psyduckSeleccionado) {
                File psyduckImg=new File("C:\\Users\\AlumnoM\\Downloads\\Poke\\src\\main\\java\\img\\psyduck.png");
                controller.mandarInfoDesdeVentana(p, pokemonRival, new Image(psyduckImg.toURI().toString()));
                cont+=1;
            }else
                if(meowthSeleccionado){
                    File meowthImg=new File("java/img/meowth.png");
                    controller.mandarInfoDesdeVentana(p2, pokemonRival, new Image(meowthImg.toURI().toString()));
                    cont2+=1;
                }else
                    if(snorlaxSeleccionado){
                        File snorlaxImg=new File("java/img/snorlax.png");
                        controller.mandarInfoDesdeVentana(p3, pokemonRival, new Image(snorlaxImg.toURI().toString()));
                        cont3+=1;
                    }else
                        if (bulbasaurSeleccionado){
                            File bulbaImg=new File("java/img/bullbasur.png");
                            controller.mandarInfoDesdeVentana(p4, pokemonRival, new Image(bulbaImg.toURI().toString()));
                            cont4+=1;
                        }else
                            if (pikachuSeleccionado){
                                File pikakImg=new File("java/img/pikachu.png");
                                controller.mandarInfoDesdeVentana(p5, pokemonRival, new Image(pikakImg.toURI().toString()));
                                cont5+=1;
                            }else
                                if (rattataSeleccionado){
                                    File rataImg=new File("java/img/rattata.png");
                                    controller.mandarInfoDesdeVentana(p6, pokemonRival, new Image(rataImg.toURI().toString()));
                                    cont6+=1;
                            }
